// Single-PTX launcher for syr2k.cl: C = alpha*(A*B' + B*A') + beta*C
// Same 2D grid as syrk; a[j*ni+k] and b[j*ni+k] are column-stride prefetch targets.
// Usage: bench_syr2k <kernel.ptx> [N]   (ni=nj=N, default 4096)
...
int main(int argc, char **argv) {
    ...
    printf("device: %s\n", devname);
    printf("ptx:    %s\n", argv[1]);
    printf("ni=%d nj=%d  A=B=%.0f MB  C=%.0f MB\n\n",
           ni, nj,
    ...
    CUfunction kern;
    CHECK(cuModuleGetFunction(&kern, mod, "__clang_ocl_kern_imp_syr2k_kernel"));

    // a[nj*ni], b[nj*ni], c[nj*nj]
    CUdeviceptr d_a, d_b, d_c;
    CHECK(cuMemAlloc(&d_a, (size_t)nj * ni * sizeof(float)));
    CHECK(cuMemAlloc(&d_b, (size_t)nj * ni * sizeof(float)));
    ...
    CHECK(cuMemcpyHtoD(d_b, h, (size_t)nj * ni * sizeof(float)));
    free(h);
    ...
    void *args[] = { &d_a, &d_b, &d_c, &alpha, &beta, &ni, &nj };
